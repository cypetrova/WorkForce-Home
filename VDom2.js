...
function DMYcheck(sDay, sMonth, sYear) {
    ...
    if (month === 2) { // Проверка за февруари
        ...
        if (day > maxDaysInFebruary) {
            alert("Не съществува такъв ден през месец Февруари!");
            return 0;
        }
    } else if ([4, 6, 9, 11].includes(month) && day > 30) { // Проверка за месеци с 30 дни
        alert("Не същестува такъв ден за избраният месец!");
        return 0;
    }
...
function D15(val1) {
    ...
    if ((document.getElementById("Maincontent_DDL14").value == '1')) {       
        if (mD15 >= age) {             
            cMTable = 1;
...
function CancelPerson() {
    if (confirm('Сигурнили сте, че искате да излезете от тази страница?')) {
        console.log('Ok');
        document.getElementById("tPerson").style.display = 'none';
